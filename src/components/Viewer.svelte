...
<script lang="ts">
    ...
    async function getGroups() {
        if (typeof $templates[selected] !== 'undefined' && typeof $templates[selected].groups === 'undefined') {
            try {
...
<section class="channels card p-1 justify-center">
	<header class="channels-header flex justify-center items-center space-x-4">
		<h3 class="h3 font-bold">Stream Viewer</h3>
	</header>
    <div id="selector" class="grid grid-cols-2 items-center justify-center text-center space-x-4 w-fit m-2">
        <div>Select Template: </div>
        {#if $templates.length > 0}
            <select class="select" bind:value={selected} on:change={() => {getGroups()}}>
    ...
    <hr class="!border-t-8 !border-double m-4" />
        ...
        <Accordion>
            {#if $templates[selected].groups.length > 0}
                {#each $templates[selected].groups as group, groupIdx (group.id)}
                    <div class="groups p-1 justify-center w-content">
                        <AccordionItem class="mb-1" key={groupIdx} bind:open={group.itemOpen}>
                            ...
                            <svelte:fragment slot="content">
                                <ViewerChannels templateIdx={selected} groupId={group.id} groupIdx={groupIdx}/>
                            </svelte:fragment>
                        </AccordionItem>
                    </div>
                {/each}
