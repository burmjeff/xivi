...
export function slide(
	node: Element,
	{
		...
		direction = 'y',
		distance = 20
	}: TransitionConfig & { direction?: 'x' | 'y'; distance?: number } = {}
): TransitionConfig {
	const style = getComputedStyle(node);
	const opacity = +style.opacity;
	const transform = style.transform === 'none' ? '' : style.transform;

	const directionProps: Record<'x' | 'y', number> =
		direction === 'y' ? { y: distance, x: 0 } : { x: distance, y: 0 };
...
export function scale(
	node: Element,
	{
		...
		start = 0.95,
		opacity = 0
	}: TransitionConfig & { start?: number; opacity?: number } = {}
): TransitionConfig {
	const style = getComputedStyle(node);
...
export function fly(
	node: Element,
	{
		...
		y = 0,
		opacity = 0
	}: TransitionConfig & { x?: number; y?: number; opacity?: number } = {}
): TransitionConfig {
	const style = getComputedStyle(node);
	const targetOpacity = +style.opacity;
	const transform = style.transform === 'none' ? '' : style.transform;
...
export function pulse(
	node: Element,
	{
		...
		easing = cubicInOut,
		intensity = 1.05
	}: TransitionConfig & { intensity?: number } = {}
): TransitionConfig {
	const transform =
