...
var (
	mediaCredentialPattern = regexp.MustCompile(`\b(?:xmk|xvt|xss|xma|xmr|xta|xtr|xtc)_[A-Za-z0-9._-]{12,}\b`)
	sensitiveValuePattern  = regexp.MustCompile(`(?i)\b(access_token|refresh_token|device_code|csrf(?:_token)?|authorization|dpop|cookie|password|secret)=([^&\s]+)`)
)
